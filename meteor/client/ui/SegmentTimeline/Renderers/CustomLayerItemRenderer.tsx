...
export class CustomLayerItemRenderer<IProps extends ICustomLayerItemProps, IState extends ISourceLayerItemState> extends React.Component<ICustomLayerItemProps & IProps, ISourceLayerItemState & IState> {
	...
	renderOverflowTimeLabel () {
		const vtContent = this.props.piece.piece.content as VTContent | undefined
		if (!this.props.piece.playoutDuration && vtContent && vtContent.sourceDuration && ((this.props.piece.renderedInPoint || 0) + vtContent.sourceDuration) > (this.props.partDuration || 0)) {
	...
	renderInfiniteItemContentEnded () {
		const content = this.props.piece.piece.content as VTContent | undefined
		const seek = content && content.seek ? content.seek : 0
		if (this.props.piece.piece.infiniteMode && content && content.sourceDuration && (this.props.piece.renderedInPoint || 0) + (content.sourceDuration - seek) < (this.props.partDuration || 0)) {
	...
	renderInfiniteIcon () {
		return (this.props.piece.piece.infiniteMode && this.props.piece.piece.infiniteMode === PieceLifespan.Infinite && !this.props.piece.playoutDuration) ?

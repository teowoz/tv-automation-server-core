...
export class VTSourceRendererBase extends CustomLayerItemRenderer<IProps & InjectedTranslateProps, IState> {
	...
	updateTime = () => {
		if (this.vPreview) {
			...
			const itemDuration = ((piece.piece.content ? piece.piece.content.sourceDuration as number : undefined) || piece.playoutDuration || piece.renderedDuration || 0)
			let targetTime = this.props.cursorTimePosition
			let seek = ((piece.piece.content ? piece.piece.content.seek as number : undefined) || 0)
			if (piece.piece.content && piece.piece.content.loop && this.vPreview.duration > 0) {
				targetTime = targetTime % (Math.min(this.vPreview.duration, itemDuration) * 1000)
			} else if (itemDuration === 0 && piece.piece.infiniteMode) {
				// noop
			} else {
